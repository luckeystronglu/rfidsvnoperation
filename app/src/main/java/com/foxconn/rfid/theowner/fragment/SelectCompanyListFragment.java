...
public class SelectCompanyListFragment extends BaseFragment implements BGARefreshLayout
        ...
        mCompanyAdapter.setOnRVItemClickListener(new BGAOnRVItemClickListener() {
            ...
            public void onRVItemClick(ViewGroup parent, View itemView, int position) {
                ...
                if (mCompanyAdapter.getItem(position).getHasChild().equals("1")) {
                    ...
                    intent.putExtra("session", PreferenceData.loadSession(getActivity()));
...
//        requestMessage.setSession(companyUser.getSession());
        requestMessage.setSession(PreferenceData.loadSession(getActivity()));
    ...
    public void onEventMainThread(EventBusMsg eventPackage) {
        super.onEventMainThread(eventPackage);
        if (eventPackage.getEmptyType() == EventBusMsg.MsgEmptyType.REFRESH_COMPANYLIST) {
            doSocket();
        }
    }
